...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	if($errmsg == '')
	{
		//store both				
		$result = mysql_query("
			INSERT INTO journal_article_category (name) VALUES ('$name') "); 	
		if($result)
		{					
			$_SESSION['notice'] = "Category Was Successfuly Saved"; 
			header('location:journal_article_category_list.php');		
		}
		else
			$errmsg .=  "There was error while Saving Article Category".mysql_error()."<br />";
	}			
}
?>
...
<form action="journal_article_category_add.php" method="post" name="form1" id="form1">
  <table width="100%" border="0" cellspacing="0">
    <tr>
      <td bordercolor="#F2F2F2" bgcolor="#F2F2F2">
	  
	    <h3><span id="writeroot"></span> Add New Category </h3></td>
      </tr>
    <tr>
    ...
    <tr>
      <td width="94%" bgcolor="#F2F2F2"><table width="100%" border="0" cellspacing="0">
        ...
        <tr>
          ...
          <td>&nbsp;</td>
          <td><input name="name" type="text" id="name" value="<?=$name;?>" size="40" /></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
